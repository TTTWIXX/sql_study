...
WHERE sn = '101'
    AND char_4 = 'SQLD         '
;
--char타입은 뒤에 공백이 있으면 공백을 넣어서 비교한다.(앞쪽에 공백이 있을경우는 다르다.)
...
SELECT 
    SYSDATE AS "현재 시간",
    SYSDATE - 1 AS "SYSDATE - 1", --하루전 날짜
    (SYSDATE + 10) - SYSDATE AS "날짜 - 날짜", --10일 후 날짜
    SYSDATE - (1/24) AS "1시간 차감", --1일을 24나 나워서 1시간!
    SYSDATE - (1/24/60) * 100 AS "100분 차감", -- 1분*100 100분차감
    SYSDATE - (1/24/60/60) * 30 AS "30초 차감" --1초*30초 30초차감 
FROM dual;
...
-- Simple expression(일치비교만됨, 대소비교가 불가)
SELECT 
    sal_cd, 
    sal_nm,
    CASE sal_cd 
...
SELECT 
    ...
    -- 없는 사람을 검색할시 존재안함으로 보여주고 싶을시 max를 걸어 null로 만든다.
FROM tb_emp
WHERE emp_nm = '이승엽';
