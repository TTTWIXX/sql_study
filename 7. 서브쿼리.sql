...
; -- 서브쿼리 안은 2row 이기때문에 오류가 난다.

-- 20200525에 받은 급여가 회사전체의 20200525 일 전체 평균 급여보다 높은 사원들의 정보(사번, 이름, 급여지급일, 받은급여액수) 조회
SELECT a.emp_no "사번",
		a.emp_nm "이름",
		b.pay_de "급여지급일",
		b.pay_amt "받은급여액수"
...
WHERE b.PAY_AMT >=(SELECT avg(PAY_AMT) 
				FROM TB_SAL_HIS
